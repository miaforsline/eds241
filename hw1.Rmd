...
\noindent In California, `r mean_pov$california_county[1]` is the county with the highest average poverty rate weighted by census tract total population. 
...
\noindent Given the equation: PM2.5 = `r mdl_clean$estimate[1]` + `r mdl_clean$estimate[2]` \* LowBirthWeight, if PM2.5 is reduced by 2, then we predict the Low Birth Rate to be approximately `r y` %.

\noindent The 95% confidence interval is bounded by `r mdl_clean$conf.high[2]` and `r mdl_clean$low[2]`, meaning we are 95% confident that the true parameter $\beta_1$ lies within this interval.
...
#robust_se2 <- starprep(mdl) #calculates standard robust errors 
#stargazer(model, se = robust_se2)

stargazer(mdl_clean, mdl2_clean, type = "latex", ci=FALSE, no.space = TRUE, 
          header = FALSE, omit = c("Constant"), omit.stat = c("adj.rsq",
                                                              "ser", 
                                                              "f",
                                                              "conf.low"),
