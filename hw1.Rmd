...
The estimated slope coefficient for PM2.5 ($\beta_1$) = `r mdl$coefficients[2]`, meaning that a 1 µg/m^3^ change in PM2.5 on average increases the low birth rate by `r mdl$coefficients[2]`. At the 5% significance level, the effect of PM2.5 on low birth rate is statistically significant because the p-value \<\<\< 0.05.
...
Given the equation: PM2.5 = `r mdl$coefficients[1]` + `r mdl$coefficients[2]` \* LowBirthWeight, if PM2.5 is reduced by 2, then we predict the Low Birth Rate to be approximately `r y`%.

The 95% confidence interval is bounded by `r ci[1]` and `r ci[2]`, meaning we are 95% confident that the true parameter $\beta_1$ lies within this interval.
...
\noindent H0: PM2.5 = Poverty

\noindent HA: PM2.5 $\neq$ Poverty

```{r, eval = FALSE}
linearHypothesis(model = mdl2, 
