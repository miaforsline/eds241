...
\noindent Drop counties with NA values for poverty
...
\noindent `r max_pov_county` is the county with the highest level of poverty in California.
...
\noindent The estimated slope coefficient for PM2.5 ($\beta_1$) = `r mdl$estimate[2]`, meaning that a 1 µg/m^3^ change in PM2.5 on average increases the low birth rate by `r mdl$estimate[2]`. At the 5% significance level, the effect of PM2.5 on low birth rate is statistically significant because the p-value \<\<\< 0.05. The standard error for PM2.5 ($\beta_1$) = `r mdl$std.error`. 
...
b1 = mdl$estimate[2]
x = -2
y = mdl$estimate[1] + mdl$estimate[2] * x
```

\noindent Given the equation: PM2.5 = `r mdl$estimate[1]` + `r mdl$estimate[2]` \* LowBirthWeight, if PM2.5 is reduced by 2, then we predict the Low Birth Rate to be approximately `r y`%.

\noindent The 95% confidence interval is bounded by `r mdl$conf.high[2]` and `r mdl$low[2]`, meaning we are 95% confident that the true parameter $\beta_1$ lies within this interval.
...
\noindent The estimated slope coefficient for Poverty ($\beta_2$) = `r mdl2$estimate[3]`, meaning that a 1% change in Poverty on average increases Low Birth Rate by `r mdl2$estimate[3]`. Compared to the prior model, the slope coefficient of PM2.5 ($\beta_1$) has decreased from `r mdl$estimate[2]`to `r mdl2$estimate[2]` because Poverty helps explain some of the change in Low Birth Weight. In other words, the prior model suffered from omitted variables bias and caused us to overestimate the impact of PM2.5 alone on Low Birth Weight.

\noindent Table 1 shows the estimated coefficients from estimating equation (1).

```{r , results = 'asis', echo = FALSE}
stargazer(mdl, mdl2, type = "latex", ci=FALSE, no.space = TRUE, 
