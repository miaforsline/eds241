...
linearHypothesis(mdl, c("price_log=-1"), white.adjust = "hc2")
```

\noindent The price elasticity of demand, or the slope of the demand curve, for sardines is approximately `r price_elasticity`. 
...
\noindent The coefficient on wind speed is approximately `r wind_coef`. In other words, on average, when wind speed increases by 1 m/sec, the log price of of sardines per kg increases by `r wind_coef` Euros. This coefficient has the expected sign because wind makes it more difficult to catch fish. 

Windier = harder to fish = affecting the supply of fish = more expensive fish 
...
stargazer(mdl2, fs1, tsls1,  
          ...
          omit = c("Constant"),  
          omit.stat = c("adj.rsq","ser", "f"),
          #covariate.labels = c("Log(Volume)", "Wind Speed (m/s)", "Log(Price)"),
          #dep.var.labels = c("Log(Price)", "Log(Volume)"),
          title = "Title", 
          table.placement = "H"
          )
```
...
left hand side of the bar = second stage (endogenous variable)
right hand side of the bar = first stage (instrument)
...
```{r}
fs4 <- lm(formula = price_log ~ wind_m_s + as.factor(country) + as.factor(year) + as.factor(month), data=data_log)

linearHypothesis(fs4, c("wind_m_s=0"), white.adjust = "hc2")
```

Wind is still a strong instrument even after we account for country, year, and month as fixed effects. 
