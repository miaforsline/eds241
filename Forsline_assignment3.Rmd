...
\noindent Table 4 shows the estimated coefficients and standard error of maternal smoking during pregnancy on infant birth weight (g) using a propensity score weighted regression (WLS). 

```{r , results = 'asis', echo = FALSE}
wls1 <- lm(formula = birthwgt ~ tobacco, data=data_p, weights=ps_weight)

tobacco_coef_weighted <- round(wls1$coefficients[[2]], digits = 2)
...
\noindent The propensity score weighted regression returns an estimate `tobacco` coefficient of `r tobacco_coef_weighted`. 