...
\noindent Therefore, it is more likely that there are significant differences in the control variables (such as mother's age) depending on if mothers smoked during pregnancy or not. For example, on average, mothers who smoked during pregnancy are `r mage_diff` years younger than mothers who did not smoke during pregnancy.
...
The estimated coefficient of tobacco is X and the standard error is Y. 
...
The linear regression analogue estimates an average treatment effect of X. (tobacco coefficient)
...
```{r}
#Exact answer changes when we apply the weights compared to the first question - what are those changes? interpret 
...
The effect of adding the weighted EPS is X. 