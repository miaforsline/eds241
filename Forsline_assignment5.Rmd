...
\noindent Since houses located near the incinerator are more likely to be older, have fewer rooms, and have less square footage (p-value < 0.05), they are already more likely to have lower housing prices than houses located far from the incinerator regardless of the incinerator's presence. In other words, the groups being compared (houses near the incincerator vs houses far from the incinerator) are not well balanced, and thus we are likely to overestimate the negative effect of the incinerator on housing values. 
...
When we control for housing and lot characteristics, the `nearinc` coefficient `r near_inc_coef2` and `year` coefficient `r year_coef` decrease and are no longer statistically significant. Thus, the incinerator indicator no longer demonstrates a causal effect on housing prices. 
...
Holding all else constant, from 1978 to 1981, housing values increased by `r year_coef`. 
