...
year_coef <- round(DD_cap3$coefficients[[3]], digits = 2) * -1
```
...
When we control for housing and lot characteristics, the `nearinc` coefficient `r near_inc_ceof2` and `year` coefficient `r year_coef` decrease and are no longer statistically significant. Thus, the incinerator indicator no longer demonstrates a causal effect on housing prices. 
...
\noindent The p-value `r p` < 0, so we can reject the null hypothesis and conclude that the house and lot characteristics are all jointly equal to zero. 
