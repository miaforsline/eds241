...
near_inc_coef <- round(DD_gen1$coefficients[[1]], digits = 2) * -1
...
Without controlling for housing or lot characteristics, the estimated DD coefficient of `nearinc` tells us that, on average, houses located near an incinerator are $`r near_inc_coef` cheaper than houses located far from an incinerator. 
...
conf <- tidy(DD_gen1, conf.int = TRUE, conf.level = 0.95)
conf_low <- conf$conf.low
conf_high <- conf$conf.high
```

The lower bound of the confidence interval is `r conf_low`, and the upper bound of the confidence interval is `r conf_high`. 
